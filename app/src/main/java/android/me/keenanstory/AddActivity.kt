...
import android.me.keenanstory.databinding.ActivityAddBinding
...
import android.me.keenanstory.api.AddResponse
...
class AddActivity : AppCompatActivity() {
    ...
    override fun onCreate(savedInstanceState: Bundle?) {
        ...
        binding.btnGallery.setOnClickListener {
            ...
            intent.type = "image/*"
            val choser = Intent.createChooser(intent, "Select image from gallery")
    ...
    private fun upload() {
        if (getFile != null) {
            ...
            if (binding.edAddDescription.text.isNullOrEmpty()) {
                Toast.makeText(this@AddActivity, "Please add a description first", Toast.LENGTH_SHORT).show()
            } else {
                showLoading(true)
            ...
            Toast.makeText(this@AddActivity, "Please enter the file first", Toast.LENGTH_SHORT).show()
        }
    }
    private fun showLoading(isLoading: Boolean) {
        if (isLoading) {
            ObjectAnimator.ofFloat(binding.buttonUpload, View.ALPHA, 0f).start()
            binding.myIndicator.visibility = View.VISIBLE
        } else {
            ObjectAnimator.ofFloat(binding.buttonUpload, View.ALPHA, 1f).start()
            binding.myIndicator.visibility = View.GONE
        }
    }
    companion object {
